...
use_true_mean = bool(int(arg6))
use_true_cov = bool(int(arg7))
num_residues = int(arg8)
...
if num_residues == 18:
    ...
    for i in rel_bin_list:
        print(i)
        subset_test_instances_df = test_instances_df[test_instances_df['orig_seq_bin_num'] == i].reset_index()
        rand_row = subset_test_instances_df.sample(n=2, random_state=bin_idx)
        print(rand_row)
        rand_idx = list(rand_row['index'])
        test_dir_idx_list.append(rand_idx[0])
        test_dir_idx_list.append(rand_idx[1])
...
if sample_conditional:
    save_dir = '%s/ddpm_samples_sampled_pdist_conditional_%s_%s_independent_dim/%s/num_samples=%d/num_repeat=%d' % (home_dir,mean_str,cov_str,num_residues_str,num_samples,num_repeat)
else:
    save_dir = '%s/ddpm_samples_sampled_pdist_unconditional_%s_%s_independent_dim/%s/num_samples=%d/num_repeat=%d' % (home_dir,mean_str,cov_str,num_residues_str,num_samples,num_repeat)
...
model_save_dir = '%s/ddpm_model_w_demb_independent_dim/%s/x' % (home_dir,num_residues_str)
...
model_save_dir = '%s/ddpm_model_w_demb_independent_dim/%s/y' % (home_dir,num_residues_str)
model_save_path = '%s/%s.pth' % (model_save_dir, model_name)
...
model_save_dir = '%s/ddpm_model_w_demb_independent_dim/%s/z' % (home_dir,num_residues_str)
model_save_path = '%s/%s.pth' % (model_save_dir, model_name)
...
pdist_filename_memmap =  '%s/pairwise_dist_sampled_%s_%s_memmap' % (save_dir,mean_str,cov_str)
...
for test_dir_idx in test_dir_idx_list: 
    ...
    ###this is a slight annoyance but:::
    ##pairwise_dist_all_bin_test is ordered by the original sequence in test_instances_df [i.e o1,m1,o2,m2,o3,m3]
    ##while pairwise_dist_sampled_true_mean_pred_cov [and other sampled variants]
    ##is ordered by the mutated sequence in test_instances_df [i.e m1,o1,m2,o2,m3,o3]
    ##so we need to correct for this off by one delta 

    if test_dir_idx % 2 == 0:
        test_dir_idx_corrected = test_dir_idx+1
    else:
        test_dir_idx_corrected = test_dir_idx-1 

    #we don't have to use convoluted instance_idx_mapping because these were generated in order wrt test_instances_df
    test_dir_start_idx = 5000*test_dir_idx_corrected 
    ...
    for i in range(0,(len(minibatch_idx_list)-1)):
        ...
        sample_minibatch_size = minibatch_idx_list[i+1]-minibatch_idx_list[i]
        ...
        sample_filename_memmap =  '%s/memmap_data/%s/sample_test_dir_idx_%d_sampled_pdist_%s_%s_x' % (home_dir, num_residues_str, test_dir_idx, mean_str, cov_str)
        ...
        sample_filename_memmap =  '%s/memmap_data/%s/sample_test_dir_idx_%d_sampled_pdist_%s_%s_y' % (home_dir, num_residues_str, test_dir_idx, mean_str, cov_str)
        ...
        sample_filename_memmap =  '%s/memmap_data/%s/sample_test_dir_idx_%d_sampled_pdist_%s_%s_z' % (home_dir, num_residues_str, test_dir_idx, mean_str, cov_str)
    ...
    sample_filename_memmap_x =  '%s/memmap_data/%s/sample_test_dir_idx_%d_sampled_pdist_%s_%s_x' % (home_dir, num_residues_str, test_dir_idx, mean_str, cov_str)
    sample_filename_memmap_y =  '%s/memmap_data/%s/sample_test_dir_idx_%d_sampled_pdist_%s_%s_y' % (home_dir, num_residues_str, test_dir_idx, mean_str, cov_str)
    sample_filename_memmap_z =  '%s/memmap_data/%s/sample_test_dir_idx_%d_sampled_pdist_%s_%s_z' % (home_dir, num_residues_str, test_dir_idx, mean_str, cov_str)
